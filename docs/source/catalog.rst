...
- ``CATALOG_DIR``: The full path to the directory containing the YAML catalog file.  This is especially useful for constructing
- (TBD) ``env(ENV_VAR)``: Access an environment variable (useful for passing tokens that should not be hardcoded into the file)
- (TBD) ``shell(CMD)``: Run a command in the shell when the template is executed and return standard out.  If the command does not have a zero return code, then an exception will be raised, aborting the operation in progress.
...
In addition to using plugins already installed in the Python environment (see :ref:`plugin-discovery`), a catalog can also use additional plugins from aribtrary locations listed in the YAML file.  Under the ``plugin:`` / ``source:`` section of the configuration file, additional plugins can be listed along with the import method:
...
- ``- dir: /my/module/directory``: All of the ``*.py`` files in this directory will be executed, and any plugin classes found will be added to the catalog's plugin registry.
...
The majority of a catalog file is a list of data sources.  Each data source has several attributes:
...
- ``driver``: Name of the Intake plugin to use with this source.  Must either be installed in the current Python environment or loaded in the ``plugin`` section of the file.
...
- ``type``: The type of the parameter.  Currently, this may be ``str``, ``int``, ``float``, ``list[str]``, ``list[int]``, ``list[float]``.  TBD: Add datetime, which accepts either a Python datatime object or an ISO8601 timestamp string.
...
Catalogs can be entirely local, in-memory objects.  A catalog file can be loaded by creating a Catalog object::
...
Intake also provides as a server which can share an Intake catalog over HTTP (or HTTPS with the help of a TLS-enabled reverse proxy).  From the user perspective, remote catalogs function identically to local catalogs::
