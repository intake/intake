...
def test_read(intake_server):
    ...
    info = d.discover()
    assert info["dtype"] == {k: str(v) for k, v in meta.dtypes.to_dict().items()}
...
def test_search(intake_server):
    ...
    remote_results = remote_catalog.search("entry1").search("part").search("part")
    local_results = local_catalog.search("entry1").search("part").search("part")
    expected = ["nested.entry1_part", "entry1_part"]
    assert isinstance(remote_results, RemoteCatalog)
    assert list(local_results) == list(remote_results) == expected
