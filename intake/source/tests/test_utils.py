...
@pytest.mark.parametrize(
    "pattern,expected",
    [
        ("data/LT05_L1TP_042033_{start_date:%Y%m%d}_{end_date:%Y%m%d}_01_T1_sr_band{band:1d}.tif", "data/LT05_L1TP_042033_*_*_01_T1_sr_band*.tif"),
        ("data/LT05_L1TP_042033_*_*_01_T1_sr_band*.tif", "data/LT05_L1TP_042033_*_*_01_T1_sr_band*.tif"),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("{year:d}/{month:d}/{day:d}.csv", "2016/2/01.csv", {"year": 2016, "month": 2, "day": 1}),
        ("{year:.4}/{month:.2}/{day:.2}.csv", "2016/2/01.csv", {"year": "2016", "month": "2", "day": "01"}),
        (
        ...
        ("data_{date:%Y_%m_%d}.csv", "data_2016_10_01.csv", {"date": datetime.datetime(2016, 10, 1, 0, 0)}),
        ("{state}{zip:5}", "PA19104", {"state": "PA", "zip": "19104"}),
        ("{state}{zip:5d}.csv", "PA19104.csv", {"state": "PA", "zip": 19104}),
        ("{state:2}{zip:d}.csv", "PA19104.csv", {"state": "PA", "zip": 19104}),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("data/{year:4d}{month:02d}{day:02d}.csv", {"year": 2016, "month": 10, "day": 1}),
    ],
)
...
def test_reverse_format_errors():
    ...
    resolved = "20012001"
    with pytest.raises(ValueError, match=("Format specifier must be set " "if no separator between fields.")):
    ...
    pattern = "data_{band}"
    resolved = "1"
    with pytest.raises(ValueError, match=("Resolved string must match " "pattern. 'data_' not found.")):
        reverse_format(pattern, resolved)
...
@pytest.mark.parametrize("pattern", ["data_{year}_{month}_{day}.csv", "data_{year:d}_{month:02d}_{day:02d}.csv", "data_{date:%Y_%m_%d}.csv"])
def test_roundtrip_reverse_formats(pattern):
    args = reverse_formats(pattern, paths)
    for i, path in enumerate(paths):
        assert pattern.format(**{field: values[i] for field, values in args.items()}) == path


@pytest.mark.parametrize(
    "path,metadata,expected",
    [
        ("http://data/band{band:1d}.tif", {"cache": [{"argkey": "urlpath", "regex": "data"}]}, "/band{band:1d}.tif"),
        ("/data/band{band:1d}.tif", {}, "/data/band{band:1d}.tif"),
        ("/data/band{band:1d}.tif", None, "/data/band{band:1d}.tif"),
    ],
)
