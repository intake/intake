...
@pytest.fixture
def file_cache():
    return FileCache("csv", {"argkey": "urlpath", "regex": "test/path", "type": "file"})
...
def test_compressions(temp_cache, tempdir, comp):
    ...
    with open(fn, "wb") as f:
        f.write(data)
    if comp == "tgz":
        subprocess.call(shlex.split("tar -czf {fn}.tgz {fn} -C {d}".format(fn=fn, d=tempdir)))
    elif comp == "tbz":
        subprocess.call(shlex.split("tar -cjf {fn}.tbz {fn} -C {d}".format(fn=fn, d=tempdir)))
    elif comp == "tar":
        subprocess.call(shlex.split("tar -cf {fn}.tar {fn} -C {d}".format(fn=fn, d=tempdir)))
    elif comp == "gz":
        subprocess.call(shlex.split("gzip " + fn))
    elif comp == "bz":
        subprocess.call(shlex.split("bzip2 " + fn))
    fn = os.path.join(tempdir, [f for f in os.listdir(tempdir) if f.startswith("data.")][0])
...
def test_dat(temp_cache):
    ...
    P = subprocess.Popen(["dat", "share", "-d", os.path.join(here, "test_dat/")])
    time.sleep(1.5)
    try:
        cat = intake.open_catalog(os.path.join(here, "cached.yaml"))
