...
@pytest.mark.skipif(sys.version_info[0] == 2, reason="Py2 exists early on argparse")
def test_help(temp_cache, env):
    out = subprocess.check_output(["intake", "cache"], env=env, universal_newlines=True)
    assert out.startswith("usage: ")

    out2 = subprocess.check_output(["intake", "cache", "-h"], env=env, universal_newlines=True)
    assert out2 == out


def test_list_keys(temp_cache, env):
    out = subprocess.check_output(["intake", "cache", "list-keys"], env=env, universal_newlines=True)
...
def test_precache(temp_cache, env):
    out = subprocess.check_output(["intake", "cache", "list-keys"], env=env, universal_newlines=True)
    assert out.startswith("[]")  # empty cache
    out = subprocess.check_output(["intake", "precache", cpath], env=env, universal_newlines=True)
...
def test_clear_one(temp_cache, env):
    ...
    keys = list(md)
    assert len(keys) == 2
    subprocess.call(["intake", "cache", "clear", keys[0]], env=env)
    md = CacheMetadata()
    assert len(md) == 1
...
def test_usage(temp_cache, env):
    ...
    out = subprocess.check_output(["intake", "cache", "usage"], env=env, universal_newlines=True)
