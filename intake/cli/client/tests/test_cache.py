...
def test_help(env):
    out = subprocess.check_output('intake cache', env=env).decode()
    assert 'usage: ' in out

    out2 = subprocess.check_output('intake cache -h', env=env).decode()
    assert out2 == out


def test_list_keys(env):
    out = subprocess.check_output('intake cache list-keys', env=env).decode()
    assert '[]' in out  # empty cache
...
def test_clear_one(env):
    ...
    keys = list(md)
    assert len(keys) == 2
    subprocess.call('intake cache clear %s'
                    '' % keys[0], env=env)
    md = CacheMetadata()
    assert len(md) == 1
...
def test_usage(env):
    ...
    out = subprocess.check_output('intake cache usage', env=env).decode()
